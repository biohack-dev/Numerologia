...
#
# File...........: numerologia.py
# Title..........: Sistema de Calculo de Numerologia Cabalistica
...
def separar_vogais_consoantes_formatado(nome):
    ...
    for letra in nome:
        if letra == ' ':
            linha_vogais.append(' ')
            linha_consoantes.append(' ')
        elif letra.upper() in vogais_set or letra.lower() in vogais_set:
            linha_vogais.append(letra)
            linha_consoantes.append(' ')
        else:
            linha_vogais.append(' ')
            linha_consoantes.append(letra)
    
    return ''.join(linha_vogais), nome, ''.join(linha_consoantes)


def separar_vogais_consoantes_lista(nome):
    """
    Retorna listas separadas de vogais e consoantes
    """
    vogais = 'AEIOUYÃÁÀÂÄÉÊËÍÌÏÎÓÔÕÖÒÚÙÛÜÇ'
    vogais_min = vogais.lower()
    vogais_set = set(vogais + vogais_min)
    ...
    letras_sem_espaco = [c for c in nome if c != ' ']
    
    for letra in letras_sem_espaco:
        if letra.upper() in vogais_set or letra.lower() in vogais_set:
            vogais_encontradas.append(letra)
        else:
            consoantes_encontradas.append(letra)
    
    return vogais_encontradas, consoantes_encontradas


def calcular_valores_vogais_consoantes(vogais, consoantes, cabalacod):
    """
    Calcula os valores numerológicos das vogais e consoantes
    """
    valor_vogais = sum(cabalacod.get(letra.upper(), 0) for letra in vogais)
...
def separar_vogais_consoantes_detalhado(nome):
    ...
    resultado.append(f"Consoantes: {' '.join(consoantes_encontradas)}")
...
# ================ Inserir dados ================
extenso = str(input(' Digite seu nome completo :'))
nome = extenso
...
print(div2*3)
print('=' * 80)
print('PIRÂMIDE NUMÉRICA DO NOME')
print('=' * 80)
print('{}'.format(namelist.replace("", "  ")[1: -1]))
print('=' * 80)

# Exibe cada nível da pirâmide centralizado
for nivel in result:
    # Calcula espaços para centralizar baseado no nível mais longo (base)
    espacos = " " * (len(result[0]) - len(nivel))
    print(espacos + " ".join(str(n) for n in nivel))
...
with open(nome_arquivo, 'w', encoding='utf-8') as arquivo:
    # Escreve cabeçalho
    arquivo.write('=' * 80 + '\n')
    arquivo.write('NUMEROLOGIA CABALÍSTICA\n')
    arquivo.write('=' * 80 + '\n\n')
    arquivo.write(f'Nome: {nome}\n')
    arquivo.write(f'Data de Nascimento: {datadenascimento}\n')
    arquivo.write('=' * 80 + '\n\n')

    arquivo.write('=' * 80 + '\n\n')

    # ===== SEPARAÇÃO DE VOGAIS E CONSOANTES =====
    arquivo.write('ANÁLISE DE VOGAIS E CONSOANTES\n')
    arquivo.write('=' * 80 + '\n\n')

    # Para o nome completo
    linha_vogais, nome_completo, linha_consoantes = separar_vogais_consoantes_formatado(nome.upper())

    arquivo.write('NOME COMPLETO:\n')
    arquivo.write(f'{linha_vogais}\n')
    ...
    # Motivação
    arquivo.write('MOTIVAÇÃO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{Motivação[0]}\n')
    arquivo.write(f'O seu número de Motivação é {motivac}.\n')
    arquivo.write(f'{Motivação[motivac]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Impressão ou Aparência
    arquivo.write('IMPRESSÃO OU APARÊNCIA\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{Impressão[0]}\n')
    arquivo.write(f'O seu número de Impressão é {aparencia}.\n')
    arquivo.write(f'{Impressão[aparencia]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Expressão
    arquivo.write('EXPRESSÃO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{Expressão[0]}\n')
    arquivo.write(f'O seu número de Expressão é {expressao}.\n')
    arquivo.write(f'{Expressão[expressao]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Destino
    arquivo.write('NÚMERO DE DESTINO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{Destino[0]}\n')
    arquivo.write(f'O seu número de Destino é {destinonum}.\n')
    arquivo.write(f'{Destino[destinonum]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Lição Cármica
    arquivo.write('LIÇÃO CÁRMICA\n')
    arquivo.write('-' * 40 + '\n')
    if licaocarm:
        arquivo.write(f'{Liçõescarmicas[0]}\n')
        arquivo.write(f'As suas lições cármicas são: {licaocarm}\n')
        for lic in licaocarm:
            arquivo.write(f'{Liçõescarmicas[lic]}\n')
    else:
        arquivo.write('Você não possui lições cármicas!\n')
    arquivo.write('\n' + '=' * 80 + '\n\n')

    # Tendência Oculta
    arquivo.write('TENDÊNCIA OCULTA\n')
    arquivo.write('-' * 40 + '\n')
    if tendocult:
        arquivo.write(f'{Tendênciasocultas[0]}\n')
        arquivo.write(f'A tendência oculta são: {tendocult}\n')
        for to in tendocult:
            arquivo.write(f'{Tendênciasocultas[to]}\n')
    else:
        arquivo.write('Você não possui Tendências Ocultas!\n')
    arquivo.write('\n' + '=' * 80 + '\n\n')

    # Resposta do Subconsciente
    arquivo.write('RESPOSTA DO SUBCONSCIENTE\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{RespostaSubconsciente[0]}\n')
    arquivo.write(f'Sua resposta do subconsciente é {respsub}.\n')
    arquivo.write(f'{RespostaSubconsciente[respsub]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Dívidas Cármicas
    arquivo.write('DÍVIDAS CÁRMICAS\n')
    arquivo.write('-' * 40 + '\n')
    if divida:
        arquivo.write(f'As Dívidas Cármicas são: {divida}\n')
        arquivo.write(f'{DívidasCármicas[0]}\n')
        for d in divida:
            arquivo.write(f'{DívidasCármicas[d]}\n')
    else:
        arquivo.write('Você não possui Dívidas Cármicas!\n')
    arquivo.write('\n' + '=' * 80 + '\n\n')

    # Missão
    arquivo.write('MISSÃO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{Missão[0]}\n')
    arquivo.write(f'O número da sua Missão é: {missao}\n')
    arquivo.write(f'{Missão[missao]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Análise do Dia de Nascimento
    arquivo.write('ANÁLISE DO DIA DE NASCIMENTO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write('O dia do nascimento é um número único, onde cada dia do mês possui suas próprias características, diferenciando-se em cada uma delas.\n')
    arquivo.write(f'O seu dia de Nascimento é: {dia}\n')
    arquivo.write(f'{DiadoNascimento[int(dia)]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Ciclo de Vida
    arquivo.write('CICLO DE VIDA\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'1º Ciclo: Numerologia {numciclo1} - Período: {ano} - {periodociclo1}, até aos {idadeciclo1} anos.\n')
    arquivo.write(f'2º Ciclo: Numerologia {numciclo2} - Período: {periodociclo1} - {periodociclo1 + 27}, dos {idadeciclo1} até aos {idadeciclo1 + idadeciclo2} anos.\n')
    arquivo.write(f'3º Ciclo: Numerologia {numciclo3} - Período: {periodociclo1 + 27} - ****, dos {idadeciclo1 + idadeciclo2} até ao final da vida.\n\n')
    
    if relacaovidacarma:
        arquivo.write('Quando um ciclo de vida tiver o mesmo número que uma das lições cármicas, esse período poderá ser um tanto conturbado.\n')
        arquivo.write('Até que a lição cármica seja aprendida e eliminada:\n')
        arquivo.write(f'Atenção ao(s) ciclo(s) cuja numerologia seja: {relacaovidacarma}\n\n')
    
    arquivo.write('DESCRIÇÃO DO SEU CICLO DE VIDA\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'1º Ciclo: Numerologia {numciclo1} - Período: {ano} - {periodociclo1}, até aos {idadeciclo1} anos.\n')
    arquivo.write(f'{PrimeiroCiclo[numciclo1]}\n')
    arquivo.write(f'2º Ciclo: Numerologia {numciclo2} - Período: {periodociclo1} - {periodociclo1 + 27}, dos {idadeciclo1} até aos {idadeciclo1 + idadeciclo2} anos.\n')
    arquivo.write(f'{SegundoCiclo[numciclo2]}\n')
    arquivo.write(f'3º Ciclo: Numerologia {numciclo3} - Período: {periodociclo1 + 27} - ****, dos {idadeciclo1 + idadeciclo2} até ao final da vida.\n')
    arquivo.write(f'{TerceiroCiclo[numciclo3]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Desafios
    arquivo.write('DESAFIOS\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write('Paralelos aos Ciclos de Vida, existem os Desafios, que nada mais são que certas fraquezas, obstáculos que se entrepõem no caminho de cada um de nós. Os Desafios representam etapas em que a pessoa tem certa dificuldade para encontrar o seu rumo e que tende a agir de forma extremada e precipitada.\n\n')
    arquivo.write(f'1º Desafio - Numerologia {desafio1}\n')
    arquivo.write(f'{Desafios[desafio1]}\n')
    arquivo.write(f'2º Desafio - Numerologia {desafio2}\n')
    arquivo.write(f'{Desafios[desafio2]}\n')
    arquivo.write(f'DESAFIO PRINCIPAL - Numerologia {desafioprinc}\n')
    arquivo.write(f'{Desafios[desafioprinc]}\n\n')
    
    for item in saude:
        for item1 in doencasi:
            if item == item1:
                arquivo.write(f'Poderá haver certos problemas de saúde durante esse período do desafio {item} com uma das seguintes partes do corpo: {saude[item]}\n')
    arquivo.write('\n' + '=' * 80 + '\n\n')

    # Momentos Decisivos
    arquivo.write('MOMENTOS DECISIVOS\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'1º Momento Decisivo - Numerologia {md1} - Período: {ano} - {periodociclo1}, até aos {idadeciclo1} anos.\n')
    arquivo.write(f'2º Momento Decisivo - Numerologia {md2} - Período: {periodociclo1} - {periodociclo1 + 9}, dos {idadeciclo1} até aos {idadeciclo1 + 9} anos.\n')
    arquivo.write(f'3º Momento Decisivo - Numerologia {md3} - Período: {periodociclo1 + 9} - {periodociclo1 + 18}, dos {idadeciclo1 + 9} até aos {idadeciclo1 + 18} anos.\n')
    arquivo.write(f'4º Momento Decisivo - Numerologia {md4} - Período: {periodociclo1 + 18} - ****, dos {idadeciclo1 + 18} até ao final da vida.\n\n')
    
    if mdmotivac == True:
        arquivo.write(f'Durante os anos do MD {motivac}, a pessoa terá os seus desejos e necessidades atendidas.\n')
    if mdexpressao == True:
        arquivo.write(f'Durante os anos do MD {expressao}, a pessoa será ajudada na sua vocação (profissão ideal).\n')
    if mdestino == True:
        arquivo.write(f'Durante os anos do MD {destinonum} em vigor, terá uma grande oportunidade de realização dos sonhos (destino).\n')
    
    if len(licaocarm) > 0:
        for q in mdlista:
            for j in licaocarm:
                if q == j:
                    arquivo.write(f'O período do {mdlista.index(q) + 1}º Momento Decisivo conterá muitas dificuldades no que relaciona ao aprendizado da lição cármica {q}, enquanto esta não for aprendida.\n')
    
    arquivo.write('\nDESCRIÇÃO DOS SEUS MOMENTOS DECISIVOS\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{MomentoDecisivo[md1]}\n')
    arquivo.write(f'{MomentoDecisivo[md2]}\n')
    arquivo.write(f'{MomentoDecisivo[md3]}\n')
    arquivo.write(f'{MomentoDecisivo[md4]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Dias favoráveis
    arquivo.write('DIAS FAVORÁVEIS\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'Dias do mês favoráveis para fechar negócios, encontros, decisões e bons resultados:\n{diafavoravel}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Relações Intervalores
    if relinterval:
        arquivo.write('RELAÇÕES INTERVALORES\n')
        arquivo.write('-' * 40 + '\n')
        arquivo.write(f'{RelaçõesInterValores[0]}\n')
        for riv in relinterval:
            arquivo.write(f'{RelaçõesInterValores[riv]}\n')
        arquivo.write('\n' + '=' * 80 + '\n\n')

    # Grau de Ascensão
    arquivo.write('GRAU DE ASCENSÃO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'{ascensao}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Número do Amor e Relacionamento
    arquivo.write('NÚMERO DO AMOR E RELACIONAMENTO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'O número do Amor e Relacionamento é: {numamor}\n')
    arquivo.write(f'{Amor[numamor]}\n')
    arquivo.write('Se o número estiver repetido, é por que essa ligação é extremamente forte.\n\n')
    arquivo.write(f'Vibra com {harmoniaconjugal[numamor - 1][1]}. É sinal de forte atração sexual (paixão) que pode,\n')
    arquivo.write('caso não se transforme em amor, levar à separação em virtude de ciúmes exagerados, inconstância sexual, arrogância de um ou ambos os parceiros.\n\n')
    
    arquivo.write(f'Atrai {harmoniaconjugal[numamor - 1][2]}. São totalmente compatíveis entre si e o amor e sexo se mesclam\n')
    arquivo.write('ardentemente. Tendem para a amabilidade, cordialidade, delicadeza e compreensão\n')
    arquivo.write('mútuas. O relacionamento com esta característica tem tudo para ser bem-sucedido,\n')
    arquivo.write('e se não for o for, ou será por interferência de terceiros ou alteração sem análise do nome\n')
    arquivo.write('(assinatura) de um dos parceiros ou mesmo de ambos.\n\n')
    
    arquivo.write(f'É oposto ao {harmoniaconjugal[numamor - 1][3]}. Por incrível que possa parecer, este não é o pior dos relacionamentos,\n')
    arquivo.write('quando um dos envolvidos tem consciência do fato e são intelectualmente desenvolvidos.\n')
    arquivo.write('Para que dure "eternamente", ambos os parceiros devem usar diplomacia constantemente e ceder\n')
    arquivo.write('(os dois) em muitos pontos, a fim de impedir o fracasso dessa união. Jamais, em hipótese alguma,\n')
    arquivo.write('um deve querer adaptar o outro aos seus ideias ou desejos. Neste caso a separação será\n')
    arquivo.write('inevitável e traumática para ambos.\n\n')
    
    arquivo.write(f'É passivo ao {harmoniaconjugal[numamor - 1][4]}. Sofrem influências, sejam elas negativas ou positivas.\n')
    arquivo.write('Este tipo de relacionamento transforma os parceiros em "amigos", em vez de\n')
    arquivo.write('"amantes". Por vezes, em virtude de uma forte vibração de um dos parceiros,\n')
    arquivo.write('o relacionamento tende a dar certo por longo tempo, ou seja, quando um ama\n')
    arquivo.write('demasiadamente e ou outro se mostra um tanto indiferente. O mais normal é que\n')
    arquivo.write('nesta configuração o relacionamento tem tendência a se "arrastar por toda a vida",\n')
    arquivo.write('ou então até que apareça a um dos envolvidos "alguém" mais interessante.\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Tabela I - Nível Universal
    arquivo.write('TABELA I - NÍVEL UNIVERSAL\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write(f'Ano Universal = {anouniversal}\n')
    arquivo.write(f'Mês Universal = {mesuniversal}\n')
    arquivo.write(f'Dia Universal = {diauniversal}\n')
    arquivo.write(f'Números que se harmonizam de acordo com o dia do nascimento: {harmoniadatanasc[harmoniadia - 1][1]}\n')
    arquivo.write(f'Números que são neutros de acordo com o dia do nascimento: {neutronasc[harmoniadia - 1][1]}\n')
    arquivo.write(f'Números que são incompatíveis de acordo com o dia do nascimento: {incompativelnasc[harmoniadia - 1][1]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Cores e seus Significados
    arquivo.write('CORES E SEUS SIGNIFICADOS\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write('O branco simboliza a luz, a alegria, o poder de Deus e a pureza; o amarelo significa o amor divino, a compreensão e a caridade; azul é a cor da eternidade, da sabedoria e da vontade; o verde significa a vida, o crescimento e a esperança; o violeta a dignidade e o saber; o vermelho é a cor das emoções; o preto simboliza a tristeza e a escuridão. A mudança do número de Expressão, não altera a cores favoráveis originais.\n')
    arquivo.write(f'Cores Favoráveis, de acordo com o número de expressão: {cores[expressao]}\n\n')
    arquivo.write('=' * 80 + '\n\n')

    # Vibração do ano
    arquivo.write('VIBRAÇÃO DO ANO\n')
    arquivo.write('-' * 40 + '\n')
    arquivo.write('A vibração de cada ano influencia a pessoa\n')
    arquivo.write(f'O seu ano pessoal é {anopessoal} e mês pessoal {mespessoal}. Sob a influência do ano {anoatual}.\n')

    arquivo.write('=' * 80 + '\n\n')

    # Rodapé
    arquivo.write('=' * 80 + '\n')
    arquivo.write('FIM DO MAPA NUMEROLÓGICO\n')
    arquivo.write(f'Gerado em: {datetime.date.today()}\n')
    arquivo.write('=' * 80 + '\n')
